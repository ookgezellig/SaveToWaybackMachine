...
urllist = []
urllist=read_local_csv("Input-Leesplein_TeArchiverenURLs_07062018.csv") # file with 1 to-be-archived Leesplein-URL per line
for url in urllist:
    #print(str(url[0]))
    ...
    if r.status_code == requests.codes.ok:
        ...
        if "text/html" in contenttype: #html page
            print(str(url[0])+ "^^^^" + base_url + r.headers['Content-Location']) # the "^^^^" is used a separator between the two URLs. The more traditional separators such as ";" or "," are not reliable enough, as the can occur in URLs.
        elif contenttype == "application/pdf": #pdf file
            print(str(url[0]) + "^^^^" + archive_url)
...
# TODO: Write everyting to a file, instead of to terminal print
