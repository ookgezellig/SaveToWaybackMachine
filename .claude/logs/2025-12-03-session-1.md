...
## CSS/JS Separation & Footer Redesign

### Code Organization (Session 2)

Split inline CSS and JavaScript from `_layouts/default.html` into separate files for better maintainability:

| File | Lines | Purpose |
|------|-------|---------|
| `assets/css/main.css` | ~760 | All site styling, KB colors, responsive, dark mode |
| `assets/js/lightbox.js` | ~200 | Lightbox with WCAG accessibility, touch/swipe support |
| `_layouts/default.html` | ~173 | Slimmed down, links to external CSS/JS |

### Footer Redesign

User requested footer to match KB style with black background and white text (referencing kb.nl screenshot):

| Element | Before | After |
|---------|--------|-------|
| Background | `#001a70` (dark blue) | `#000` (black) |
| Text color | `#e0e0e0` | `#fff` (white) |
| Headings | `#cba052` (gold) | `#fff` (white) |
| Links | `#96bded` (light blue) | `#fff` (white) |
| Link hover | `#fff` | `#ccc` (light gray) |
| Social icons | Gold circles (`#8f6a2a`) | Transparent (no circles) |
| KB Logo | `Logo_koninklijke_bibliotheek.svg` | `Online_wit_Nederlands_KB-logo_0.svg` (white) |
| Bottom border | `#8f6a2a` (gold dark) | `#333` (dark gray) |

### Commits This Session

| Commit | Description |
|--------|-------------|
| `e49f31f` | Split CSS and JS into separate files, update footer to black background |
| `7536c28` | Fix footer styling: all white text, no gold circles, white logo |
| `6ba68db` | Update session log with CSS/JS split and footer redesign details |
| `2cfb517` | Update timeline, add new phases, improve compliance page title |
| `fdc5e35` | Add white KB logo, update phase headers with session info |
| `62146cf` | Fix sentence case in tools section |
| `fa362c9` | Remove duplicate time investment from TOC |
| `820528b` | Update page title references and remove license link from footer |
| `28b449b` | Fix session dates: phases 1-5 in session 1, phases 6-7 in session 2 |
| `5b821e3` | Remove title case from phase headers and WCAG guidelines |
| `89506c3` | Update session log with latest commits |
| `66d1bd2` | Fix phase dates: all session 1 phases on Dec 2 |
| `d752fd5` | Fix title case in Results table headers |
| `791a329` | Update timeline table: all session 1 phases on Dec 2 |
| `f24b500` | Update session log with latest commits |
| `f37f599` | Update README: 10+ hours, 33+ commits, Playwright |
| `ea258aa` | Add anchor links to timeline table phases |
| `4393cb4` | Fix phase 7 name in timeline table to match header |
| `b68bcdf` | Update session log with latest commits |
| `d2a8b87` | Add frequent commits section, dedicated KB huisstijl page, session 2 log link |
| `a88512a` | Add session 2 log to sitemap |

---

## Documentation Updates

- Updated `how-this-site-was-built.md`:
  - Timeline overview: added Date column, new phases 6 & 7
  - Updated totals to ~10 hours and 33 commits
  - Added Phase 6 (KB Huisstijl & Compliance) and Phase 7 (Code Organization)
  - Clarified phases 1-5 happened in session 1
  - Fixed tools: Selenium → Playwright
  - Removed camelCase from Schema.org terms
  - Updated footer colors to reflect black/white design

- Renamed `compliance.md` title to "Accessibility, privacy & licensing"

- Added missing white KB logo SVG (`assets/Online_wit_Nederlands_KB-logo_0.svg`)

- Fixed sentence case throughout (removed unnecessary title case)

- Updated sitemap.md statistics (33 commits, ~10 hours)

- Removed "License (CC0 1.0)" link from footer

- Updated all references to compliance page title across site (footer, sitemap, TOC)
